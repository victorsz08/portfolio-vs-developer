        ...
        <main>
          <header 
            className="w-full z-[1000] fixed
               ...
               max-[864px]:px-8 max-[864px]:py-6 max-[864px]:justify-between max-[480px]:relative max-[480px]:px-5">
            <div>
                <h1 className="font-bold text-xl text-gray-200">
                    VS <strong className="font-bold text-bold text-purple-600">DEV</strong>
                </h1>
            </div>
            <div className="max-[480px]:block hidden">
            <IoMenu className='w-5 h-5 text-gray-50 cursor-pointer' onClick={() => setIsOpen(!isOpen)}/>
            </div>
            <nav className="flex items-center text-sm text-gray-400 font-light justify-center gap-5 max-[480px]:hidden">
          ...
          <div className={`w-screen p-8 top-16 bg-black text-gray-200 text-sm flex flex-col items-end gap-4 overflow-hidden animate-fade-down ${isOpen ? 'max-h-60 py-4' : 'hidden'}`}>
